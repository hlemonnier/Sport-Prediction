...
function resolvePythonCommand() {
  const configured = String(process.env.SPORT_PREDICTION_PYTHON ?? "").trim();
...
function isSportDir(dirPath) {
  ...
  if (
    ["Research", "research", "papers", "projects", ".git"].includes(name)
  ) {
    return false;
  }
...
async function main() {
  ...
  const dataDir = process.env.SPORT_PREDICTION_API_DATA_DIR
    ? path.resolve(process.env.SPORT_PREDICTION_API_DATA_DIR)
