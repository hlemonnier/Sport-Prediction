...
from research.experiments.boundary_20260908.telemetry_verification_v2 import verify as v
...
def test_fit_lock_uses_frozen_runner_external_selection_label_contract():
    original = Path(v.__file__).parent.parent/'telemetry_verification'/'verify.py'
    before = original.read_text()
    expected = before.replace("fit_lock['selection_labels_read'] is False", "fit_lock['external_selection_labels_attached'] is False")
    assert Path(v.__file__).read_text() == expected
    import ast
    source = ast.parse(Path(v.__file__).read_text())
    guards = [node for node in ast.walk(source) if isinstance(node, ast.Assert)
              and "external_selection_labels_attached" in ast.unparse(node)]
    assert len(guards) == 1
    code = compile(ast.fix_missing_locations(ast.Module(body=guards, type_ignores=[])), '<schema regression>', 'exec')
    exec(code, {'data_lock': {'model_fits': 0}, 'fit_lock': {'external_selection_labels_attached': False}})
    with pytest.raises(AssertionError):
        exec(code, {'data_lock': {'model_fits': 0}, 'fit_lock': {'external_selection_labels_attached': True}})
    with pytest.raises(KeyError):
        exec(code, {'data_lock': {'model_fits': 0}, 'fit_lock': {'selection_labels_read': False}})
