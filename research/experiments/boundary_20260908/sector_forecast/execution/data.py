...
def _prefix(checkpoint):
    ...
    if s1['sequence']>=checkpoint['packet_sequence']:
        # The pilot can legitimately repeat an unchanged S1 in the S2 packet.
        # Recover its first prior S1 from immutable earlier pilot rows in build.
        raise ValueError('S2 needs its prior S1 source, not an equal-packet display repeat')
    if s1['available_ms']>checkpoint['checkpoint_ms']:raise ValueError('Future S1 source')
    return [_finite(s1['value'],'Prior S1',True),current]
...
def build_event(paths,event_key):
    ...
    for checkpoint in checkpoint_rows:
        ...
        if status==ISSUED_STATUS:
            ...
            if checkpoint['sector']==2 and checkpoint['received_driver_fields']['Sector1']['sequence']==checkpoint['packet_sequence']:
                earlier=next((r for r in reversed(prior_s1[prefix_key]) if r['packet_sequence']<checkpoint['packet_sequence']),None)
                if earlier is None or earlier['packet_sequence']>=checkpoint['packet_sequence'] or earlier['sector_seconds']!=checkpoint['received_driver_fields']['Sector1']['value']:
                    raise ValueError('An S2 same-packet S1 revision lacks matching immutable prior support')
                point_checkpoint={**checkpoint,'received_driver_fields':{**checkpoint['received_driver_fields'],'Sector1':{'value':earlier['sector_seconds'],'available_ms':earlier['checkpoint_ms'],'sequence':earlier['packet_sequence']}}}
            prefix=_prefix(point_checkpoint)
            ...
            features=encode(point_checkpoint,history,ctx,result)
