...
def load_event(directory, event, data_lock_entry=None):
    ...
    for row in rows:
        ...
        if row["status"] == "issued":
            ...
            # Frozen run.safe_json writes unavailable NaN features as null.
            require(all(x is None or (type(x) in (int, float) and not math.isinf(x)) for x in row["features"].values()),
