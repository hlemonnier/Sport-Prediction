...
Status: implementation and independent pre-fit review in progress. No real sector model has yet been fitted or scored in this execution. This status will be replaced by the recorded outcome after the immutable run finishes; earlier published prototype snapshots remain unchanged.
...
```sh
PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. OMP_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 MKL_NUM_THREADS=1 .venv-f1/bin/python -m research.experiments.boundary_20260908.sector_forecast.execution.run freeze
PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. OMP_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 MKL_NUM_THREADS=1 .venv-f1/bin/python -m research.experiments.boundary_20260908.sector_forecast.execution.run build
PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. OMP_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 MKL_NUM_THREADS=1 .venv-f1/bin/python -m research.experiments.boundary_20260908.sector_forecast.execution.run select
```
