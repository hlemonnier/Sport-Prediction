# Amended execution v2 review and preserved v1 assessment

The historical review below is preserved verbatim from `execution/mathematical_review.md` (SHA256 `3219c74abfea58741e838d464730eb39c3e2088f439289d9ec3e09e9078f260a`). Its readiness statements, test counts and source hashes describe **v1 before the failed target-free assembly**. They do not describe v2 readiness or measured model performance. The independent v2 amendment assessment follows the preserved review.

<!-- BEGIN VERBATIM HISTORICAL V1 REVIEW -->
...
<!-- END VERBATIM HISTORICAL V1 REVIEW -->

## Independent v2 amendment assessment — 2026-09-08

**Ready for a new tested source lock and discovery execution.** No concrete blocker remains in the reviewed v2 runner, source pairing, atomic S1 amendment, feature/model invariants or evaluation gates. This is a code and synthetic-test conclusion. This reviewer has not run a historical v2 build, read canonical outcomes for v2, fitted a candidate or inspected a selection score. The reviewer also authored the feature encoder; the independent scope here is the subsequent assembly amendment, runner wiring and byte comparisons, not independent feature design.

### Failure and exact correction

The original design lock is `8550844b86f14a8742140872f55f005b96ab27386ac533185222ad560f83aa26`. Its 21 source bindings still matched during this review. The original receipt records 215 passing tests; the build then failed before its full data lock, labels, learned-model fitting or predictive scoring. Twelve persisted target-free event ledgers account for 25,360 positive sector updates. Their reference/Gaussian point calculations are target-free computations, not measured predictive gains.

The preserved raw diagnostic is `execution_preflight/same_packet_s1_failure_diagnosis.json`, SHA256 `e27cbb4b442ef60aedb35f7e7145d582c8e4c3df03c908dc69d85720ebcec64c`; its follow-up binding/plan has SHA256 `d83a20276d57f6cc6795c97bbd181db6cbdf891fcb25036ecf7244500489a846`. The failed v1 build receipt has SHA256 `0842f3ef051678a45da2935386d3e6299e60568f2a96c4cf203cd7938deedecb`. These files sit under `artifacts/research/boundary_20260908/sector_forecast/` and were checked without reading targets or scores.

The concrete trigger is event 202213, driver 47: packet 24356 reports S1 = 30.526 seconds; packet 24357 corrects S1 to 30.486 while reporting S2 = 31.233. V1 required the current S1 value to equal its earlier value, so it rejected the otherwise supported atomic snapshot. This is a genuine provider revision, not loss of the earlier S1 from the cache.

V2 separates two roles. The first positive S1 in the same pilot epoch must come from a strictly earlier packet and satisfy the pilot's attribution prerequisite. The value used at S2 is the latest positive S1 in the actual current snapshot, with its actual source sequence and availability clock. In the example the observed prefix is 30.486 + 31.233 = 61.719 seconds. The earlier S1 forecast remains unchanged. A same-clock earlier physical packet may establish the S1 prerequisite, consistent with the pilot; completed-lap history still requires a strictly earlier availability clock.

Using the **first** S1 prerequisite matches `pilot.first[1]`. Selecting the latest preceding S1 checkpoint instead could introduce an unnecessary dependency on that revision's own checkpoint status. The new code keeps the latest value source separate and records both roles in `source_provenance.s1_prefix`. Missing, wrong-epoch, ambiguous or future prerequisite/source inputs fail closed. An initial same-packet S1/S2 without an earlier S1 remains unsupported under the original pilot gate. Raw timestamp regression is sticky within the pilot epoch; v2 does not revive later S2 checkpoints in such an epoch. Current S3 and future packets remain excluded from the prefix encoder.

### Unchanged scientific design and runtime bindings

Fresh comparisons found `features.py`, `feature_contract.json` and `evaluate.py` byte-identical between v1 and v2. The four reference formulas and Gaussian/HGB model implementation remain the same shared, locked parent modules. The specification's candidate list, HGB parameters, reference list, evaluation equations, selection/substantial-gain gates, chronological split, complete discovery event set and transfer manifest are exactly unchanged. Only execution identity, the textual local feature-contract path and the explicit unscored atomic-correction amendment differ in the specification.

Import inspection confirmed that v2 `run`, `data`, `evaluate` and `features`, including the lazy feature encoder, resolve to `execution_v2`. The runner's repository root is correct and its default output is `artifacts/research/boundary_20260908/sector_forecast/execution_v2`; its implementation otherwise differs from v1 only in that output directory. The source inventory binds the v2 leaf files and shared parent dependencies without accidentally binding v1 executable leaf modules. Original contract/draft references remain intentional historical provenance links. A caller-specified output directory must remain new and immutable.

### Verification and final reviewed hashes

The complete v2 execution test directory passed **133 synthetic tests in 1.46 seconds** in this independent review. The 33 assembler tests are included in that total. Coverage includes genuine atomic corrections, unchanged repeats, prior revisions, first-S1 prerequisite semantics, sticky regressions, same-packet field ordering, current-S3/future-payload poisoning, immutable earlier forecasts, target identity and common model population. The fixed feature and evaluator regression suites also passed.

```sh
PYTHONPATH=. PYTHONDONTWRITEBYTECODE=1 OMP_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 MKL_NUM_THREADS=1 .venv-f1/bin/python -m pytest -q --import-mode=importlib -p no:cacheprovider research/experiments/boundary_20260908/sector_forecast/execution_v2
```

Files below are relative to `research/experiments/boundary_20260908/sector_forecast/execution_v2/`:

| File | SHA256 |
| --- | --- |
| run.py | e76c40ad96b556159faeb7aedaa85b90e2ad8c3fefaab1d8c5ecfc5d53146aa7 |
| specification.json | 5eb7f4829567e264c8f5b22ea5c65cd731544b25650f7ed4bce038c285ee9180 |
| data.py | d65014adf1e2f3c7c465de33435fc24aced3d2790882e656e39e887712103ef1 |
| test_data.py | ac3baf4a2de9cdb2f0a90ed1564064542e14d3ad38eaf6363ba73198c2f17a5d |
| data_contract.json | 36716ea96c5d8eb9a51a569af5fd509390ff7c9ae161600e334eb298a9dc0457 |
| features.py | 4052f16e67e842286171ffe2df1124babf8bfca9cc442d253d8bd1e144593cef |
| feature_contract.json | b3398db61b856cf2f564c43ab6ebc7713ffc4c870b0b5b6c4f49801c92b9e53f |
| evaluate.py | 6be893a852495df15b4d3222d8e1ffcd27d73c171f8d95c4e4cf1b15ba9821e5 |

The new pre-fit lock must bind these final sources and this review before historical v2 assembly. The original failed attempt and its copied review stay immutable. There is no new empirical sector gain or production-activation claim in this assessment.

Suggested commit: `fix(research): preserve atomic sector revisions at forecast issuance`.
