...
Status: **v2 source only; its historical replay has not run**. The preserved
v1 run encountered leading UTF-8 BOM bytes in both streams and stopped all 3,454
snapshots before consuming a forecast packet. That is a parser-boundary failure,
not evidence that gap data are absent. This sibling changes only recognition of
one optional BOM at absolute byte zero; all other field, cutoff and readiness
semantics are unchanged. This is a conditional next information audit while the independent CPC
...
Exactly one optional UTF-8 BOM is recognized only at the start of the file.
Repeated or midstream BOMs remain invalid timestamp boundaries. After that, only
the next exact 12-byte ASCII timestamp header is read before admission;
...
```sh
PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. .venv-f1/bin/python -m pytest -q --import-mode=importlib -p no:cacheprovider research/experiments/boundary_20260908/rival_gap_pilot_v2/test_pilot.py
PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. .venv-f1/bin/python -m research.experiments.boundary_20260908.rival_gap_pilot_v2.pilot freeze --review PATH_TO_APPROVED_REVIEW
PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. .venv-f1/bin/python -m research.experiments.boundary_20260908.rival_gap_pilot_v2.pilot run
```

Suggested commit: `research(f1-live): handle leading BOM in rival-gap feasibility replay`.
