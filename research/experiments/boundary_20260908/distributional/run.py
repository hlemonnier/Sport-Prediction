...
from research.experiments.boundary_20260908.distributional import model

ROOT=mechanics.ROOT
HERE=Path(__file__).resolve().parent
OUT=ROOT/'artifacts/research/boundary_20260908/distributional'
OLD=mechanics.OLD
...
def specification():
    spec=json.loads(SPEC.read_text());lock=json.loads((OUT/'design_lock.json').read_text())
...
def discover():
    ...
    for item in inherited['input_manifest']:assert sha(ROOT/item['path'])==item['sha256']
    ...
    result={'selected':candidate,'all_selection_metrics':metrics,'relative_wis_gain':gains,'advancement_passed':bool(advanced),
        ...
        'source_files':sources(),'source_sha256':digest(sources()),'design_lock_sha256':sha(OUT/'design_lock.json'),'completed_at_utc':now()}
...
def transfer():
    ...
    for item in inherited['input_manifest']:
        for name in ['laps','metadata']:
            if name in item and item[name] is not None:
                value=item[name]
                if isinstance(value,dict) and 'path' in value:assert sha(ROOT/value['path'])==value['sha256']
    with threadpool_limits(limits=1):
