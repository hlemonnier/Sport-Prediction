...
def _mc_position_distribution(
    ...
    max_mc_work = 250000
    work = len(driver_ids) * int(horizon_laps) * requested_samples
    reduction_reason: Optional[str] = None
    if work > max_mc_work and len(driver_ids) > 0 and horizon_laps > 0:
        effective_samples = max(200, int(max_mc_work // (len(driver_ids) * int(horizon_laps))))
        reduction_reason = (
            f"work={work} exceeded max_mc_work={max_mc_work}; "
            f"mc_samples reduced from {requested_samples} to {effective_samples}"
        )
    else:
